...
class PedClass {
    ...
    firstSpawn() {
        ...
        alt.emitServer("pedSyncer:client:firstSpawn", this);
    }
    ...
    async respawn() {
        ...
        this.scriptID = native.createPed(this.gender=="male"?4:5, this.model, this.pos.x, this.pos.y, this.pos.z, native.getHeadingFromVector2d(this.rot.x, this.rot.y), false, false);
    ...
    outOfRange() {
        ...
        //Tell the server that this ped is no longer in the range of this client
        alt.emitServer("pedSyncer:client:outOfRange", this.id);
    }
    ...
    static sendMyPedPoses() {
        ...
        for (let ped of Ped.getAllStreamedPeds().filter(p => p.netOwner == alt.Player.local.id)) pedsToSend.push(ped);
    ...
    /**
     * Check if any ped is out of the streaming-range - if so, handle the outOfRange of the ped
     */
    static checkPedOutOfRange() {
        //Iterate over all peds which are streamed in filtered by these peds which are not in a distance of 150
        for (let ped of Ped.getAllStreamedPeds().filter(p => !inDistanceBetweenPos(p.pos, alt.Player.local.pos, 150))) ped.outOfRange();

        alt.setTimeout(Ped.checkPedOutOfRange, 1000);
    }
